...
- **`/my_stats`** — личная статистика (регистрация, план, сообщения/токены: всего и за день).
- **`/admin ...`** — админские команды (доступ только для `ADMIN_IDS`).
  - Основные админ-команды в новом формате: `/admin_status`, `/admin_stats`, `/admin_user`, `/admin_sub_grant`, и т.д. (слова разделяются `_`).
  - Подробное описание команд: см. `ADMIN_COMMANDS.md`.
